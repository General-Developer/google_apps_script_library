...
import 'package:supabase_gas/src/constants.dart';
import 'package:supabase_gas/src/version.dart';
import 'package:supabase_gas/supabase.dart';
...
class SupabaseClient {
  ...
  SupabaseClient(
    ...
    _authHttpClient = AuthHttpClient(_supabaseKey, httpClient ?? Client(), _getAccessToken);
    ...
    if (accessToken == null) {
      _log.config('Initialize SupabaseClient v$version with no custom access token');
      _listenForAuthEvents();
    } else {
      _log.config('Initialize SupabaseClient v$version with custom access token');
    }
  }
    ...
    if (authInstance.currentSession?.isExpired ?? false) {
      try {
        ...
        if (expiresAt != null) {
          ...
          final isExpiredWithoutMargin = DateTime.now().isAfter(DateTime.fromMillisecondsSinceEpoch(expiresAt * 1000));
