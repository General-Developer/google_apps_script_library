...
import 'package:supabase_gas/fork/postgrest/postgrest.dart';
...
void main() {
  ...
  group("Default http client", () {
    ...
    test('set header on rpc', () async {
      ...
      await postgrest.rpc('empty-succ').setHeader("myKey", "myValue").select().head();
      ...
      expect(httpClient.lastRequest!.headers, isNot(containsPair("myKey", "myValue")));
    });
    ...
    test('set header on query builder', () async {
      ...
      await postgrest.from('empty-succ').setHeader("myKey", "myValue").select().head();
      expect(httpClient.lastRequest!.headers, containsPair("myKey", "myValue"));
